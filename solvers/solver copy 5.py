...
def choose_word(expected_info_ams):
    ...
    for i in words_list:
        ...
        if(res_points < points):
            ...
            res_words = i
            candi = [res_words]
        elif(res_points == points):
            candi.append(res_words)
    print(list(zip(ranking,ranking_words)))
    if(len(candi) > 1):
        ind = exists_list(ans_candidate,candi)
        print(ans_candidate)
        print(ind)
        if(ind != -1):
            print(ans_candidate[ind])
...
def calculate_ams_benefit(state,index,char):
    ...
    if(state[index][char] == NO_EXIST):
        ...
        if(word_has_char == 0 or word_has_no_char == 0):
            return 0
