    ...
    for i in words_list:
        ...
        for k in range(0,len(used)):
            for l in range(k+1,len(used)):
                ...
                ruijido = (x_not_y + not_x_y +x_and_y + not_x_not_y)/norm
...
def calculate_ams_benefit(state,index,char):
    ...
    if(state[index][char] == NO_EXIST):
        ...
        if(word_has_no_char == 0 or word_has_no_char == 0):
            return 0
        return min(word_has_char,word_has_no_char)/max(word_has_char,word_has_no_char)+(word_has_pos_char/(len(ans_candidate)*5))
        ...
        if(word_has_char == 0 or word_has_pos_char == 0):
            return 0
        return min(word_has_char,word_has_pos_char)/max(word_has_char,word_has_pos_char)
...
killer_case = ['daunt', 'freak', 'gamer', 'gaunt', 'gazer', 'happy', 'hatch', 'haunt', 'impel', 'jaunt', 'maker', 'paint', 'taker', 'vaunt', 'wafer', 'wager']
